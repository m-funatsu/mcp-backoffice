...
const SKILLS_DATA = [
  ...
  // 専門スキル
  { name: '財務分析', category: '専門スキル', type: 'domain_specific' },
  { name: '法務知識', category: '専門スキル', type: 'domain_specific' },
  { name: 'マーケティング', category: '専門スキル', type: 'domain_specific' },
  { name: '営業戦略', category: '専門スキル', type: 'domain_specific' },
  { name: '顧客対応', category: '専門スキル', type: 'domain_specific' },
  { name: '品質管理', category: '専門スキル', type: 'domain_specific' },
  { name: 'リスク管理', category: '専門スキル', type: 'domain_specific' },
  { name: '業務改善', category: '専門スキル', type: 'domain_specific' }
];
...
const TRAINING_COURSES = [
  ...
  { name: '英語ビジネス', category: 'ソフトスキル', type: 'external', duration: 60 },
...
class DemoDataGenerator {
  ...
  async insertTrainingCourses() {
    ...
    for (const course of TRAINING_COURSES) {
      ...
      await this.run(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `, [courseId, course.name, course.category, course.type, course.duration, '全社員', `${course.name}に関する知識・スキル習得`, true]);
  ...
  async insertPerformanceEvaluations() {
    ...
    for (const employee of this.employees) {
      ...
      for (const period of evaluationPeriods) {
        ...
        await this.run(`
          INSERT INTO performance_evaluations (evaluation_id, employee_id, evaluator_id, evaluation_period, evaluation_type, evaluation_date, overall_rating, performance_score, retention_risk_level, promotion_readiness, status)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [evaluationId, employee.id, evaluatorId, period, evaluationType, evaluationDate.toISOString().split('T')[0], overallRating, performanceScore, retentionRisk, promotionReadiness, 'finalized']);
  ...
  async insertEngagementSurveys() {
    ...
    await this.run(`
      INSERT INTO employee_engagement_surveys (survey_id, survey_name, survey_type, survey_period, questions, launch_date, close_date, participation_rate, response_rate)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    for (const employee of this.employees) {
      if (Math.random() < 0.78) { // 78%の回答率
        ...
        await this.run(`
          INSERT INTO survey_responses (response_id, survey_id, employee_id, responses, response_date, overall_satisfaction, enps_score, engagement_score, wellbeing_score)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        }), responseDate.toISOString().split('T')[0], overallSatisfaction, enpsScore, engagementScore, wellbeingScore]);
